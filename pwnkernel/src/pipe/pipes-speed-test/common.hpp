...
static void parse_options(int argc, char **argv, Options &options)
{
    struct option long_options[] = {
        ...
        {"csv",                 no_argument, 0, 0},
        {"write_to_file",       no_argument, 0, 0},
        {"splice_from_file",    required_argument, 0, 0},
        {0, 0, 0, 0}};
    ...
    while (true)
    {
        ...
        else if (c == 0)
        {
            ...
            options.write_to_file = options.write_to_file || (strcmp("write_to_file", option) == 0);

            if (strcmp("splice_from_file", option) == 0)
            {
                options.in_splice_file = (char *)malloc(16);
                sscanf(optarg, "%16s", options.in_splice_file);
            }
...
NOINLINE UNUSED static char *allocate_buf(const Options &options)
{
    void *buf = NULL;
